...
//            // add object
//            arctic = world_->addArticulatedSystem(resourceDir_+"/arctic/waffleiron/waffleiron.urdf", "", {}, raisim::COLLISION(0), raisim::COLLISION(0)|raisim::COLLISION(1)|raisim::COLLISION(2)|raisim::COLLISION(63));
...
//            std::cout<<"gcDim_obj: "<<gcDim_obj<<std::endl;
//            std::cout<<"gvDim_obj: "<<gvDim_obj<<std::endl;
...
//            freezedTarget_obj_.setZero(gcDim_obj);
...
//            arctic->setGeneralizedCoordinate(Eigen::VectorXd::Zero(gcDim_obj));
            ...
            /// Initialize reward
//            Yaml::Node cfg_l = Yaml::GetNode("/home/huizhang/intern/raisim_grasp_arctic/raisimGymTorch/raisimGymTorch/env/envs/two_hands_arti/cfgs/cfg_reg.yaml");
//            Yaml::Node cfg_r = Yaml::GetNode("/home/huizhang/intern/raisim_grasp_arctic/raisimGymTorch/raisimGymTorch/env/envs/two_hands_arti/cfgs/cfg_trail.yaml");
//            rewards_r_.initializeFromConfigurationFile(cfg_r["environments"]["reward"]);
//            rewards_l_.initializeFromConfigurationFile(cfg_l["environments"]["reward"]);
...
//            /// Set standard properties
            ...
            obj_weight_t_ = arctic->getMass(top_id);
            obj_weight_b_ = arctic->getMass(bottom_id);
//            double rand_weight = (std::rand()%500)/float(10000) - 0.025;
////            std::cout<<rand_weight<<std::endl;
//            obj_weight_t_ += rand_weight;
//            rand_weight = (std::rand()%500)/float(10000) - 0.025;
////            std::cout<<rand_weight<<std::endl;
//            obj_weight_b_ += rand_weight;
//            arctic->setMass(top_id, obj_weight_t_);
//            arctic->setMass(bottom_id, obj_weight_b_);
            obj_weight_ = obj_weight_t_ + obj_weight_b_;
...
//                gvDim_obj = arctic->getDOF();
        ...
        void reset_state(const Eigen::Ref<EigenVec>& init_state_r,
            ...
            objPgain.setZero();
            objDgain.setZero();
//            std::cout<<"gcDim_obj1: "<<gcDim_obj<<std::endl;
//            std::cout<<"gvDim_obj1: "<<gvDim_obj<<std::endl;
            arctic->setPdGains(objPgain, objDgain);
...
//            raisim::Mat<3,3> temp_ori;
            raisim::matmul(Body_orientation_l.e().transpose(), Obj_orientation_set_b, init_obj_ori_wrist_l);
//            raisim::RotmatToEuler(temp_ori, init_obj_ori_l); // obj_pose_: object pose in wrist frame
                         ...
                         const Eigen::Ref<EigenVec>& goal_qpos_r)
        {
            ...
            for(int i = 0; i < num_bodyparts; i++){
//                Position_r[0] = ee_goal_pos_r[i*3] - final_obj_pos_t_[0];
//                Position_r[1] = ee_goal_pos_r[i*3+1] - final_obj_pos_t_[1];
//                Position_r[2] = ee_goal_pos_r[i*3+2] - final_obj_pos_t_[2];
//
                          ...
                          const Eigen::Ref<EigenVec>& goal_contacts_r)
        {
            ...
            Eigen::VectorXd obj_goal_pos_r_cast;
            obj_goal_pos_r_cast = obj_goal_pos_r.cast<double>();
//            if(right_kind_idx == 9 && left_kind_idx == 8){
//                obj_goal_pos_r_cast[7] = 0.0;
//            }
            set_gc_for_arctic(final_obj_pos_b_, final_obj_pos_t_, obj_goal_pos_r_cast);
//            set_gc_for_arctic(final_obj_pos_b_, final_obj_pos_t_, obj_goal_pos_r);
            /// set final object pose
...
//                Position_l[2] = ee_goal_pos_l[i*3+2] - final_obj_pos_b_[2];
            ...
            //obj_goal_pos_true[3] = 1;
            //obj_goal_pos_true[4] = 0;
            //obj_goal_pos_true[5] = 0;
            //obj_goal_pos_true[6] = 0;
...
//            else if (right_kind_idx == 9 && grasp_num_r > 40){
...
//                pTarget_r_.head(6) = pTarget_r_bk.head(6);
//            }
...
//            else if (left_kind_idx == 9 && grasp_num_l > 40){
...
//                pTarget_l_.head(6) = pTarget_l_bk.head(6);
//            }
...
//            pTarget_r_ = action_r.cast<double>();
//            if (!root_guided_r && !motion_synthesis) {
//                raisim::Vec<3> wrist_act, temp;
//                raisim::matvecmul(Obj_orientation_temp_t, action_r.head(3), temp);
...
//            pTarget_l_ = action_l.cast<double>();
//            pTarget_l_ = pTarget_l_.cwiseProduct(actionStd_l_); //residual action * scaling
////            pTarget_l_.tail(nJoints_) += actionMean_l_.tail(nJoints_); //add wrist bias (first 3DOF) and last pose (48DoF)
////            pTarget_l_.head(3) += freezedTarget_l_.head(3);
//            pTarget_l_ += freezedTarget_l_;
...
//
//            mano_l_->getFramePosition(body_parts_l_[0], Position_l); // Position: body position of a body in mano
//            mano_l_->getFrameOrientation(body_parts_l_[0], Body_orientation_l);
//            Eigen::Vector3d temp_force_bias, temp_force_bias_b, temp_force_bias_t;
//            temp_force_bias.setZero();
//            temp_force_bias_b.setZero();
//            temp_force_bias_t.setZero();
//            temp_force_bias[2] = obj_weight_ * 9.81;
//            temp_force_bias_b[2] = obj_weight_b_ * 9.81;
//            temp_force_bias_t[2] = obj_weight_t_ * 9.81;
//            temp_force_bias = Body_orientation_l.e().transpose() * temp_force_bias;
//            temp_force_bias_b = Body_orientation_l.e().transpose() * temp_force_bias_b;
//            temp_force_bias_t = Body_orientation_l.e().transpose() * temp_force_bias_t;
////            pTarget_clipped_l.head(3) = pTarget_clipped_l.head(3) + temp_force_bias / 500;
//
//            std::vector<raisim::Vec<3>> coms = arctic->getBodyCOM_W();
//            raisim::Vec<3> com_dis_b, com_dis_t;
//            Eigen::Vector3d temp_torque_bias_b, temp_torque_bias_t;
//            temp_torque_bias_b.setZero();
//            temp_torque_bias_t.setZero();
//            com_dis_b = coms[0] - Position_l;
//            com_dis_t = coms[1] - Position_l;
//            com_dis_b = Body_orientation_l.e().transpose() * com_dis_b;
//            com_dis_t = Body_orientation_l.e().transpose() * com_dis_t;
//            temp_torque_bias_b = com_dis_b.e().cross(temp_force_bias_b);
//            temp_torque_bias_t = com_dis_t.e().cross(temp_force_bias_t);
////            pTarget_clipped_l.segment(3,3) = pTarget_clipped_l.segment(3,3) + (temp_torque_bias_b + temp_torque_bias_t)/50;
//            std::cout<<(freezedTarget_l_.head(6) - actionMean_l_.head(6)).transpose()<<std::endl;
//            std::cout<<(temp_force_bias/500).transpose()<<std::endl;
//            std::cout<<((temp_torque_bias_t + temp_torque_bias_b)/50).transpose()<<std::endl<<std::endl;
...
//            pose_reward_l_ = -(gc_l_.segment(3, 3) - initial_gc_l.segment(3, 3)).squaredNorm();
//            pos_reward_l_ = -(gc_l_.head(3) - initial_gc_l.head(3)).squaredNorm();

//            std::cout<<obj_trans_init_b.norm()<<std::endl;
//            std::cout<<rel_obj_pose_world_b_.norm()<<std::endl;
//            std::cout<<rel_obj_goal_pos_b_.norm()<<std::endl<<std::endl;
//            if ((rel_obj_pose_world_b_.norm() < 0.5)&&(rel_obj_goal_pos_b_.norm() < 0.1)){
//                obj_angle_reward_ = - rel_obj_angle_.norm();
//            }
//            else{
//                obj_angle_reward_ = -10;
//            }
            obj_angle_reward_ = - rel_obj_angle_.norm();
...
//            rel_obj_pos_reward_l_ = rel_obj_pos_b.squaredNorm();
...
//            contact_reward_l_ = contacts_l_.sum();
...
//            obj_pos_reward_l_ = -rel_obj_init_b.e().squaredNorm();
...
//            obj_pose_reward_r_ = acos(0.5*(diff_obj_pose_mat.e().trace() - 1));
...
//                    std::cout<<"position "<<Position_r<<std::endl;
...
//                    obj_trans_init_b = rel_obj_init_t;
...
//                if (contact_list_obj.size() <= contact.getPairContactIndexInPairObject()) continue;
//                if (contact_list_obj[contact.getPairContactIndexInPairObject()].getlocalBodyIndex() != bottom_id) continue;
...
//            Eigen::VectorXd obj_pose6d_r_, obj_pose6d_l_, obj_pose_diff_wrist_6d_l_;
//            obj_pose6d_r_.setZero(6);
//            obj_pose6d_l_.setZero(6);
//            obj_pose_diff_wrist_6d_l_.setZero(6);
//            raisim::Mat<3,3> obj_pose_diff_mat_in_wrist_r, obj_bottom_init_mat, obj_bottom_init_mat_trans, obj_bottom_diff_mat, obj_bottom_diff_wrist_mat_r, obj_bottom_diff_wrist_mat_l;
//
...
//            raisim::Vec<4> temp_quat;
//            raisim::eulerToQuat(obj_pose_diff_wrist, temp_quat);
//            raisim::quatToRotMat(temp_quat, obj_pose_diff_mat_in_wrist_r);

//            obj_pose_diff_wrist_6d_l_.segment(0,3) = obj_pose_diff_wrist_mat.e().col(0);
//            obj_pose_diff_wrist_6d_l_.segment(3,3) = obj_pose_diff_wrist_mat.e().col(1);
//
//            Eigen::Vector3d z_world, z_world_wrist_l, z_world_wrist_r;
//            raisim::Mat<3,3> hand_ori_mat_l, hand_ori_mat_r;
//            z_world.setZero();
//            z_world_wrist_l.setZero();
...
//                    obj_pose6d_r_, //6, bottom pose change in wrist frame
//                    z_world_wrist_r, // 3, world z axis in wrist frame
...
//                    wrist_pose_obj_r_.e(); // wrist pose in object frame (3)
                    //rel_obj_pos_wrist_r_.e(), // object position relative to goal in wrist frame
                    //rel_obj_pose_r_; // object pose relative to goal in wrist frame
...
//                std::cout<<"fuck off the nan"<<std::endl;
                return true;
            }
