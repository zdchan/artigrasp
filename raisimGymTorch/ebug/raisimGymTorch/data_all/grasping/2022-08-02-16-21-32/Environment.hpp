...
namespace raisim {
    ...
    public:
        ...
        explicit ENVIRONMENT(const std::string& resourceDir, const Yaml::Node& cfg, bool visualizable) :
            ...
            mano_l_ = world_->addArticulatedSystem(resourceDir+"/mano_double/"+hand_model_l,"",{},raisim::COLLISION(0),raisim::COLLISION(0)|raisim::COLLISION(1)|raisim::COLLISION(2)|raisim::COLLISION(63));
            ...
            joint_limits_ = mano_l_->getJointLimits();
...
//                std::cout << " box pos" << box->getPosition() << std::endl;
        ...
        void reset_state(const Eigen::Ref<EigenVecDouble>& init_state_r, 
                         const Eigen::Ref<EigenVecDouble>& init_state_l, 
                         const Eigen::Ref<EigenVecDouble>& init_vel_r, 
                         const Eigen::Ref<EigenVecDouble>& init_vel_l, 
                         const Eigen::Ref<EigenVecDouble>& obj_pose) final {
            ...
            gc_set_r_.tail(nJoints_-3) = init_state_r.tail(nJoints_-3); //.cast<double>();
            gv_set_r_ = init_vel_r; //.cast<double>();
            gc_set_l_.head(6).setZero();
            gc_set_l_.tail(nJoints_-3) = init_state_l.tail(nJoints_-3); //.cast<double>();
            gv_set_l_ = init_vel_l; //.cast<double>();
        ...
        void set_goals(const Eigen::Ref<EigenVecDouble>& obj_goal_angle,
                       const Eigen::Ref<EigenVecDouble>& obj_goal_pos,
                       const Eigen::Ref<EigenVecDouble>& ee_goal_pos_r,
                       const Eigen::Ref<EigenVecDouble>& ee_goal_pos_l,
                       const Eigen::Ref<EigenVecDouble>& goal_pose_r,
                       const Eigen::Ref<EigenVecDouble>& goal_pose_l,
                       const Eigen::Ref<EigenVecDouble>& goal_qpos_r,
                       const Eigen::Ref<EigenVecDouble>& goal_qpos_l,
                       const Eigen::Ref<EigenVecDouble>& goal_contacts_r,
                       const Eigen::Ref<EigenVecDouble>& goal_contacts_l) 
        final {
            ...
            final_obj_test = obj_goal_pos;
            final_obj_test[7] = 0.7;
            set_gc_for_arctic(final_obj_b_test, final_obj_t_test, final_obj_test);


            Eigen::VectorXd final_qpos_l = goal_qpos_l; 
            set_gc_for_arctic(final_obj_pos_b_, final_obj_pos_t_, obj_goal_pos);
            ...
            quat_obj_init_b = final_obj_pos_b_.tail(4); //quat_obj_init: orientation of object goal pose, in quaternion
            quat_obj_init_t = final_obj_pos_t_.tail(4);
            ...
            final_pose_r_ = goal_pose_r;
            final_pose_r_.head(3) = euler_goal_pose.e(); // change the orientation of hand final pose to the object frame

            raisim::eulerToQuat(goal_pose_l.head(3), quat_goal_hand_w);
            raisim::quatToRotMat(quat_goal_hand_w, root_pose_world_l_);

            /// Compute and set object relative goal hand pose
            raisim::quatToRotMat(quat_obj_init_t, rotm_goal_t);
            raisim::transpose(rotm_goal_t, rotm_goal_tran_t);
            raisim::matmul(rotm_goal_tran_t, root_pose_world_l_, rotm_goal_hand_r);
            ...
            final_pose_l_ = goal_pose_l;
            final_pose_l_.head(3) = euler_goal_pose.e();
            ...
            for(int i = 0; i < num_bodyparts; i++){
                Position_r[0] = ee_goal_pos_r[i*3] - final_obj_pos_b_[0];
                Position_r[1] = ee_goal_pos_r[i*3+1] - final_obj_pos_b_[1];
                Position_r[2] = ee_goal_pos_r[i*3+2] - final_obj_pos_b_[2];

                raisim::matvecmul(Obj_orientation_b, Position_r, Rel_fpos);

                final_ee_pos_r_[i*3] = Rel_fpos[0];
                final_ee_pos_r_[i*3+1] = Rel_fpos[1];
                final_ee_pos_r_[i*3+2] =  Rel_fpos[2]; // change the ee pose to object frame
            }
            for(int i = 0; i < num_bodyparts; i++){
                ...
                Position_l[0] = ee_goal_exact_l[0] - final_obj_pos_t_[0];
                Position_l[1] = ee_goal_exact_l[1] - final_obj_pos_t_[1];
                Position_l[2] = ee_goal_exact_l[2] - final_obj_pos_t_[2];

                raisim::matvecmul(Obj_orientation_t, Position_l, Rel_fpos);
            ...
            num_active_contacts_r_ = double(goal_contacts_r.sum());
            num_active_contacts_l_ = double(goal_contacts_l.sum()); 
            final_contact_array_r_ = goal_contacts_r;
            final_contact_array_l_ = goal_contacts_l;
            ...
            obj_goal_pos_true = obj_pos_init_;
        ...
        float* step(const Eigen::Ref<EigenVecDouble>& action_r, const Eigen::Ref<EigenVecDouble>& action_l) final {
            ...
            if (motion_synthesis)
            {
                ...
                raisim::eulerToQuat(final_pose_l_.head(3), final_pose_quat_l);
                raisim::quatToRotMat(final_pose_quat_l, rotm_goal_hand_r);
            ...
            /// The following applies the wrist guidance technique (compare with paper)
                ...
                raisim::matvecmul(Obj_orientation_temp_t, final_ee_pos_l_.head(3), Fpos_world_l);
                raisim::vecadd(Obj_Position_t, Fpos_world_l);
                ...
                raisim::eulerToQuat(final_pose_l_.head(3), final_pose_quat_l);
                raisim::quatToRotMat(final_pose_quat_l, rotm_goal_hand_r);
                raisim::matmul(Obj_orientation_temp_t, rotm_goal_hand_r, rotm_goal_hand_w);
                raisim::matmul(init_or_l_, rotm_goal_hand_w, rotm_goal_hand_h);
                raisim::RotmatToEuler(rotm_goal_hand_h, act_pose_l);
            ...
            pTarget_l_ = action_l;
            //pTarget_l_[0] = -pTarget_l_[0];
            //for(int i = 1; i < num_joint; i++) {
            //    pTarget_l_[3*i+1] = -pTarget_l_[3*i+1];
            //    pTarget_l_[3*i+2] = -pTarget_l_[3*i+2];
            //}
            ...
            rel_obj_reward_r_ = rel_obj_vel_b.squaredNorm();
            ...
            rel_obj_reward_l_ = rel_obj_vel_t.squaredNorm();
        ...
        void updateObservation() {
            ...
            raisim::matmul(palm_world_pose_mat_trans_l, Obj_orientation_temp_t, obj_pose_wrist_mat_l);
            //raisim::matmul(Right2Left, obj_pose_wrist_mat_l, mat_temp);
            //raisim::matmul(mat_temp, Right2Left, mat_rot_left);
            ...
            for(int i = 0; i < num_bodyparts ; i++){
                ...
                if (i==0)
                {
                    ...
                    rel_objpalm_l[0] = Position_l[0]-Obj_Position_t[0];
                    rel_objpalm_l[1] = Position_l[1]-Obj_Position_t[1];
                    rel_objpalm_l[2] = Position_l[2]-Obj_Position_t[2];
                    ...
                    rel_obj_pos_r_ = Body_orientation_r.e().transpose()*rel_obj_init_b.e(); // object displacement from initial position in wrist coordinates
                    rel_obj_pos_l_ = Body_orientation_l.e().transpose()*rel_obj_init_t.e();
                    //rel_obj_pos_l_[0] = -rel_obj_pos_l_[0];

                    raisim::matmul(Obj_orientation_b, Body_orientation_r, rot_mult_r); // current global wirst pose in object relative frame
                    raisim::RotmatToEuler(rot_mult_r, euler_hand_r);
                    raisim::matmul(Obj_orientation_t, Body_orientation_l, rot_mult_l); 
                    ...
                    rel_obj_vel_b = Body_orientation_r.e().transpose() * Obj_linvel_b.e(); // object velocity in wrist frame
                    rel_obj_qvel_b = Body_orientation_r.e().transpose() * Obj_qvel_b.e();
                    rel_obj_vel_t = Body_orientation_l.e().transpose() * Obj_linvel_t.e();
                    //rel_obj_vel_t[0] = -rel_obj_vel_t[0];
                    rel_obj_qvel_t = Body_orientation_l.e().transpose() * Obj_qvel_t.e();
                    ...
                    raisim::quatToRotMat(final_obj_pos_b_.segment(3,4), final_obj_rotmat_temp);
                    raisim::matmul(Obj_orientation_b, final_obj_rotmat_temp, rel_pose_mat);   // deprecated

                    raisim::quatToEuler(final_obj_pos_b_.segment(3,4), final_obj_euler); // final_obj_euler: 
                    ...
                    raisim::matmul(init_or_l_, Obj_orientation_temp_t, obj_wrist); // object pose in wrist frame
                    raisim::transpose(obj_wrist, obj_wrist_trans);
                    raisim::matmul(final_obj_wrist, obj_wrist_trans, diff_obj_pose_mat);
                    raisim::RotmatToEuler(diff_obj_pose_mat, rel_obj_pose_r3);
                ...
                Position_r[0] = Position_r[0] - Obj_Position_b[0];
                Position_r[1] = Position_r[1] - Obj_Position_b[1];
                Position_r[2] = Position_r[2] - Obj_Position_b[2]; // relative pose from body position to object position
                raisim::matvecmul(Obj_orientation_b, Position_r, Rel_fpos); // compute current relative pose in object coordinates
                ...
                Position_l[0] = Position_l[0] - Obj_Position_t[0];
                Position_l[1] = Position_l[1] - Obj_Position_t[1];
                Position_l[2] = Position_l[2] - Obj_Position_t[2];
                raisim::matvecmul(Obj_orientation_t, Position_l, Rel_fpos);
                ...
                raisim::matvecmul(Obj_orientation_temp_b, obj_frame_diff_r, obj_frame_diff_w); // convert distances to world frame
                raisim::matvecmul(body_orientation_transpose_r, obj_frame_diff_w, obj_frame_diff_h_r); // convert distances to wrist frame
                raisim::matvecmul(Obj_orientation_temp_t, obj_frame_diff_l, obj_frame_diff_w); // convert distances to world frame
                ...
                if (visualizable_)
                {
                    raisim::matvecmul(Obj_orientation_temp_b, {final_ee_pos_r_[i * 3], final_ee_pos_r_[i * 3 + 1], final_ee_pos_r_[i * 3 + 2]}, sphere_pos);
                    vecadd(Obj_Position_b, sphere_pos);
                    spheres[2*i]->setPosition(sphere_pos.e());
                    raisim::matvecmul(Obj_orientation_temp_t, {final_ee_pos_l_[i * 3], final_ee_pos_l_[i * 3 + 1], final_ee_pos_l_[i * 3 + 2]}, sphere_pos);
            ...
            for(auto& contact: mano_l_->getContacts()) {
                ...
                if (contact_list_obj.size() <= contact.getPairContactIndexInPairObject()) continue;
            ...
            obDouble_r_ << gc_r_,      // (mirror) 51, generalized coordinate
                    bodyLinearVel_r_,  // (mirror) 3, wrist linear velocity
                    bodyAngularVel_r_, // (mirror) 3, wrist angular velocity
                    ...
                    rel_obj_vel_b,  // (x mirror) 3, object velocity in wrist frame
                    rel_obj_qvel_b, // (yz mirror) 3, object angular velocity in wrist frame
        ...
        void observe(Eigen::Ref<EigenVecDouble> ob_r, Eigen::Ref<EigenVecDouble> ob_l) final {
            ob_r = obDouble_r_;
            ob_l = obDouble_l_;
        }
        ...
        void set_rootguidance() final {
            ...
            mano_l_->setPdGains(jointPgain, jointDgain); // set PD gains for manos
    ...
    private:
        ...
        Eigen::Vector3d bodyLinearVel_r_, bodyLinearVel_l_, bodyLinearVel_l_obs_, bodyAngularVel_r_, bodyAngularVel_l_, bodyAngularVel_l_obs_, rel_obj_qvel_b, rel_obj_qvel_t, rel_obj_vel_b, rel_obj_vel_t, up_pose_r, up_pose_l, rel_body_table_pos_r_, rel_body_table_pos_l_;
