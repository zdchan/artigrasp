...
namespace raisim {
    ...
    public:
        ...
        explicit ENVIRONMENT(const std::string& resourceDir, const Yaml::Node& cfg, bool visualizable) :
            ...
            gc_r_imitate.setZero(gcDim_);
            gc_l_imitate.setZero(gcDim_);
            imitation_pose_l_.setZero(gcDim_);
            imitation_pose_r_.setZero(gcDim_);

            gc_l_.setZero(gcDim_);
            gv_l_.setZero(gvDim_);
            gc_set_l_.setZero(gcDim_); gv_set_l_.setZero(gvDim_);
            gc_world_r_.setZero(gcDim_);
            gc_world_l_.setZero(gcDim_);
            ...
            actionMean_l_bk.setZero(actionDim_);  actionMean_l_bias.setZero(actionDim_);
            actionMean_r_bk.setZero(actionDim_); actionMean_r_bias.setZero(actionDim_);
            ...
            obj_goal_pos_true.setZero(8);
            stage_dim.setZero(); stage_pos.setZero();
            init_or_r_.setZero(); init_or_l_.setZero(); init_rot_r_.setZero(); init_rot_l_.setZero();
            init_root_r_.setZero(); init_root_l_.setZero(); init_obj_.setZero(); init_obj_2.setZero(); init_obj_rot_.setZero();
            ...
            obj_goal_pos_b_dis_.setZero(3);
            obj_goal_pos_t_dis_.setZero(3);
        ...
        void add_stage(const Eigen::Ref<EigenVec>& stage_dim_input,
            ...
            stage_dim = stage_dim_input.head(3).cast<double>();
            stage_pos = stage_pos_input.head(3).cast<double>();
        }
        ...
        void load_articulated(const std::string& obj_model){
            ...
            if (visualizable_) {
                arcticVisual = server_->addVisualArticulatedSystem("arctic_visual", resourceDir_+"/arctic/"+obj_model, 0, 1, 0, 1);
            }
        ...
        void reset_right_hand(const Eigen::Ref<EigenVec>& init_state_r,
            ...
            gc_set_r_.tail(nJoints_-3) = gc_r_.tail(nJoints_-3).cast<double>();; //.cast<double>();
            gv_set_r_.head(6).setZero();
            gv_set_r_.tail(nJoints_-3) = gv_r_.tail(nJoints_-3).cast<double>();; //.cast<double>();
            gc_set_l_.head(6).setZero();
            gc_set_l_.tail(nJoints_-3) = gc_l_.tail(nJoints_-3).cast<double>();; //.cast<double>();
            gv_set_l_.head(6).setZero();
            gv_set_l_.tail(nJoints_-3) = gv_l_.tail(nJoints_-3).cast<double>();; //.cast<double>();

            auto idx = mano_r_->getBodyIdx("right_wrist_rz");

            mano_r_->getPosition(idx, init_root_r_);
            mano_r_->getOrientation(idx, init_rot_r_);
            raisim::transpose(init_rot_r_, init_or_r_);

            mano_r_->setBasePos(init_root_r_);
            mano_r_->setBaseOrientation(init_rot_r_);
            mano_r_->setState(gc_set_r_, gv_set_r_);

            idx = mano_l_->getBodyIdx("left_wrist_rz");

            mano_l_->getPosition(idx, init_root_l_);
            mano_l_->getOrientation(idx, init_rot_l_);
            raisim::transpose(init_rot_l_, init_or_l_);
            ...
            obj_pos_init_b_.head(3) = obj_goal_trans_b.e();
            obj_pos_init_t_.head(3) = obj_goal_trans_t.e();
            obj_pos_init_b_.segment(3,4) = obj_goal_ori_b.e();
            obj_pos_init_t_.segment(3,4) = obj_goal_ori_t.e();

            updateObservation();

            freezedTarget_l_.head(6).setZero();
            actionMean_r_.segment(3,3).setZero();
        }
        ...
        void reset_state(const Eigen::Ref<EigenVec>& init_state_r,
            ...
            imitate_count_l = 0;
            imitate_count_r = 0;
            actionMean_l_bias.setZero(actionDim_);
            actionMean_r_bias.setZero(actionDim_);
            actionMean_r_bk.setZero(actionDim_);
            actionMean_l_bk.setZero(actionDim_);
            ...
            Eigen::VectorXd obj_pose_cast, obj_pose_cast2, obj_pose_cast3;
            obj_pose_cast = obj_pose.cast<double>().head(8);
            ...
            obj_pos_init_  = obj_pose.cast<double>().head(8); // 8 dof
        ...
        {
            Eigen::VectorXd obj_pos_bk;
            obj_pos_bk = arctic->getGeneralizedCoordinate().e();
            ...
            Eigen::VectorXd gc_r_bk, gv_r_bk;
            mano_r_->getState(gc_r_bk, gv_r_bk);
            ...
            arctic->setGeneralizedCoordinate(obj_pos_bk);
            mano_r_->setBasePos(init_root_r_);
            mano_r_->setBaseOrientation(init_rot_r_);
            mano_r_->setGeneralizedCoordinate(gc_r_bk);
        }
        ...
        final {
            ...
            Eigen::VectorXd gc_l_bk, gv_l_bk;
            mano_l_->getState(gc_l_bk, gv_l_bk);
            ...
            mano_l_->setBasePos(init_root_l_);
            mano_l_->setBaseOrientation(init_rot_l_);
            mano_l_->setGeneralizedCoordinate(gc_l_bk);
        }

        void set_obj_goal(const Eigen::Ref<EigenVec>& obj_goal_angle,
                          const Eigen::Ref<EigenVec>& obj_goal_pos)
        {
            Eigen::VectorXd current_obj_state;
            obj_goal_pos_true = obj_goal_pos.cast<double>();
            obj_goal_pos_true[7] = obj_goal_angle[0];
            current_obj_state = arctic->getGeneralizedCoordinate().e();
            Eigen::VectorXd current_obj_t, current_obj_b;
            current_obj_t.setZero(7);
            current_obj_b.setZero(7);
            set_gc_for_arctic(final_obj_pos_b_, final_obj_pos_t_, obj_goal_pos_true);
            set_gc_for_arctic(current_obj_b, current_obj_t, current_obj_state);
            if (visualizable_) {
        ...
        float* step(const Eigen::Ref<EigenVec>& action_r, const Eigen::Ref<EigenVec>& action_l) final {
            ...
            if (ms_l){
                ...
                actionMean_l_bk.head(3) = actionMean_l_.head(3);
                ...
                actionMean_l_bk.segment(3,3) = actionMean_l_.segment(3,3);
                actionMean_l_.segment(3,3) += gc_l_.segment(3,3);
                actionMean_l_.head(6) += actionMean_l_bias.head(6);
            ...
            if (ms_r){
                ...
                actionMean_r_bk.head(3) = actionMean_r_.head(3);
                ...
                actionMean_r_bk.segment(3,3) = actionMean_r_.segment(3,3);
                actionMean_r_.segment(3,3) += gc_r_.segment(3,3);

                actionMean_r_.head(6) += actionMean_r_bias.head(6);
            ...
            if (imitate_l_world){
                /// set actionMean as the pregrasp pose
                raisim::Mat<3,3> rotm_goal_hand_l, rotm_goal_hand_w, rotm_goal_hand_h;
                raisim::Vec<4> final_pose_quat_l;
                raisim::Vec<3> act_pose_l;
                Eigen::VectorXd obj_state(8);

                imitate_count_l += 1;

                actionMean_l_.head(3) = gc_l_imitate.head(3) + (gc_world_l_.head(3) - gc_l_imitate.head(3)) * std::min(1.0,(0.02*imitate_count_l));
                actionMean_l_.segment(3,3) = gc_l_imitate.segment(3,3) + (gc_world_l_.segment(3,3) - gc_l_imitate.segment(3,3)) * std::min(1.0,(0.02*imitate_count_l));
                actionMean_l_.tail(nJoints_-3) = gc_l_imitate.tail(nJoints_-3) + (gc_world_l_.tail(gcDim_-6) - gc_l_imitate.tail(gcDim_-6)) * std::min(1.0,(0.02*imitate_count_l));
            }
            ...
            if (imitate_r_world){
                ...
                actionMean_r_.head(3) = gc_r_imitate.head(3) + (gc_world_r_.head(3) - gc_r_imitate.head(3)) * std::min(1.0,(0.02*imitate_count_r));
                actionMean_r_.segment(3,3) = gc_r_imitate.segment(3,3) + (gc_world_r_.segment(3,3) - gc_r_imitate.segment(3,3)) * std::min(1.0,(0.02*imitate_count_r));
                actionMean_r_.tail(nJoints_-3) = gc_r_imitate.tail(nJoints_-3) + (gc_world_r_.tail(gcDim_-6) - gc_r_imitate.tail(gcDim_-6)) * std::min(1.0,(0.02*imitate_count_r));
            }
            ...
            if (imitate_r){
                ...
                /// Convert final root hand translation back from (current) object into world frame
                raisim::matvecmul(Obj_orientation_temp_b, imitation_pose_r_.head(3), Fpos_world_r);
                raisim::vecadd(Obj_Position_b, Fpos_world_r);

                raisim::vecsub(Fpos_world_r, init_root_r_, act_pos_r); // compute distance of current root to initial root in world frame
                raisim::matvecmul(init_or_r_, act_pos_r, act_or_pose_r); // rotate the world coordinate into hand's origin frame (from the start of the episode)

                raisim::eulerToQuat(imitation_pose_r_.segment(3,3), final_pose_quat_r);
                raisim::quatToRotMat(final_pose_quat_r, rotm_goal_hand_r);
                raisim::matmul(Obj_orientation_temp_b, rotm_goal_hand_r, rotm_goal_hand_w);
                ...
                pose_diff = act_pose_r.e() - gc_r_.segment(3,3);
                for (int i = 0; i < 3; i++) {
                    if (pose_diff[i] > pi_)
                        pose_diff[i] -= 2*pi_;
                    else if (pose_diff[i] < -pi_)
                        pose_diff[i] += 2*pi_;
                }

                actionMean_r_.head(3) = gc_r_imitate.head(3) + (act_or_pose_r.e() - gc_r_imitate.head(3)) * std::min(1.0,(0.02*imitate_count_r));
                actionMean_r_.segment(3,3) = gc_r_.segment(3,3) + (pose_diff) * std::min(1.0,(0.02*imitate_count_r));
                actionMean_r_.tail(nJoints_-3) = gc_r_.tail(nJoints_-3) + (imitation_pose_r_.tail(gcDim_-6) - gc_r_.tail(gcDim_-6)) * std::min(1.0,(0.02*imitate_count_r));
            }
            ...
            if (imitate_l){
                ...
                /// Convert final root hand translation back from (current) object into world frame
                raisim::matvecmul(Obj_orientation_temp_b, imitation_pose_l_.head(3), Fpos_world_l);
                raisim::vecadd(Obj_Position_b, Fpos_world_l);

                raisim::vecsub(Fpos_world_l, init_root_l_, act_pos_l); // compute distance of current root to initial root in world frame
                raisim::matvecmul(init_or_l_, act_pos_l, act_or_pose_l); // rotate the world coordinate into hand's origin frame (from the start of the episode)

                raisim::eulerToQuat(imitation_pose_l_.segment(3,3), final_pose_quat_l);
                raisim::quatToRotMat(final_pose_quat_l, rotm_goal_hand_l);
                raisim::matmul(Obj_orientation_temp_b, rotm_goal_hand_l, rotm_goal_hand_w);
                raisim::matmul(init_or_l_, rotm_goal_hand_w, rotm_goal_hand_h);
                raisim::RotmatToEuler(rotm_goal_hand_h, act_pose_l);

                imitate_count_l += 1;

                Eigen::Vector3d pose_diff;
                pose_diff = act_pose_l.e() - gc_l_.segment(3,3);
                for (int i = 0; i < 3; i++) {
                    if (pose_diff[i] > pi_)
                        pose_diff[i] -= 2*pi_;
                    else if (pose_diff[i] < -pi_)
                ...
                actionMean_l_.head(3) = gc_l_imitate.head(3) + (act_or_pose_l.e() - gc_l_imitate.head(3)) * std::min(1.0,(0.02*imitate_count_l));
                actionMean_l_.segment(3,3) = gc_l_.segment(3,3) + (pose_diff) * std::min(1.0,(0.02*imitate_count_l));
                actionMean_l_.tail(nJoints_-3) = gc_l_.tail(nJoints_-3) + (imitation_pose_l_.tail(gcDim_-6) - gc_l_.tail(gcDim_-6)) * std::min(1.0,(0.02*imitate_count_l));
            }
            ...
            else if (imitate_r || imitate_r_world){
                pTarget_r_ = actionMean_r_;
            }
            else if (motion_synthesis || ms_r){
            ...
            if(root_keep_l){
                pTarget_l_ = gc_set_l_;
            }
            else if (imitate_l || imitate_l_world){
                pTarget_l_ = actionMean_l_;
            }
            else if (motion_synthesis_l || ms_l){
            ...
            if (right_kind_idx == 7){
                ...
                rewards_r_.record("obj_vel_reward_", std::max(-10.0, obj_vel_reward_r_));
                rewards_r_.record("obj_qvel_reward_", std::max(-10.0, obj_qvel_reward_r_));
        ...
        void updateObservation() {
            ...
            rel_obj_goal_pos_b_[0] = final_obj_pos_b_[0] - Obj_Position_b[0];
            rel_obj_goal_pos_b_[1] = final_obj_pos_b_[1] - Obj_Position_b[1];
            rel_obj_goal_pos_b_[2] = final_obj_pos_b_[2] - Obj_Position_b[2];
            rel_obj_goal_pos_t_[0] = final_obj_pos_t_[0] - Obj_Position_t[0];
            rel_obj_goal_pos_t_[1] = final_obj_pos_t_[1] - Obj_Position_t[1];
            rel_obj_goal_pos_t_[2] = final_obj_pos_t_[2] - Obj_Position_t[2];

            obj_goal_pos_b_dis_[0] = final_obj_pos_b_[0] - Obj_Position_b[0];
            obj_goal_pos_b_dis_[1] = final_obj_pos_b_[1] - Obj_Position_b[1];
            obj_goal_pos_b_dis_[2] = final_obj_pos_b_[2] - Obj_Position_b[2];
            obj_goal_pos_t_dis_[0] = final_obj_pos_t_[0] - Obj_Position_t[0];
            obj_goal_pos_t_dis_[1] = final_obj_pos_t_[1] - Obj_Position_t[1];
            obj_goal_pos_t_dis_[2] = final_obj_pos_t_[2] - Obj_Position_t[2];

            //distance from current orientation to final orientation, in world coord
            raisim::quatToRotMat(final_obj_pos_b_.segment(3,4), final_obj_pose_mat);
            raisim::transpose(Obj_orientation_temp_b, obj_orientation_temp_trans_b);
            raisim::matmul(final_obj_pose_mat, obj_orientation_temp_trans_b, diff_obj_pose_mat);
            raisim::RotmatToEuler(diff_obj_pose_mat, rel_obj_pose_world_b_);

            raisim::quatToRotMat(final_obj_pos_t_.segment(3,4), final_obj_pose_mat);
            raisim::transpose(Obj_orientation_temp_t, obj_orientation_temp_trans_t);
            raisim::matmul(final_obj_pose_mat, obj_orientation_temp_trans_t, diff_obj_pose_mat);
...
//            // relative distance in wrist frame
            ...
            raisim::matmul(palm_world_pose_mat_trans_r, Obj_orientation_temp_t, obj_pose_wrist_mat_r); //修改
            ...
            for(int i = 0; i < num_bodyparts ; i++){
                ...
                if (i==0)
                {
                    ...
                    rel_objpalm_r[0] = Position_r[0]-Obj_Position_t[0];  //修改
                    ...
                    rel_objpalm_l[0] = Position_l[0]-Obj_Position_b[0];  //修改
                    ...
                    rel_obj_pos_l_ = Body_orientation_l.e().transpose()*rel_obj_init_b.e(); //修改
                    ...
                    raisim::matmul(Obj_orientation_b, Body_orientation_l, rot_mult_l);  //修改
                    ...
                    raisim::quatToRotMat(final_obj_pos_b_.segment(3,4), final_obj_pose_mat); //修改 // final_obj_pose_mat: final object orientation in matrix
                ...
                raisim::matvecmul(Obj_orientation_temp_t, obj_frame_diff_r, obj_frame_diff_w); //修改// convert distances to world frame
            ...
            for(auto& contact: contact_list_obj) {
                if (contact.skip() ||
                    ...
                    continue;
                // check if this object is object A
            ...
            raisim::Mat<3,3> wrist_ori_r, wrist_ori_l;
            mano_r_->getFrameOrientation(body_parts_r_[0], wrist_ori_r);
            mano_l_->getFrameOrientation(body_parts_l_[0], wrist_ori_l);
            Eigen::Vector3d rotation_axis_obj, rotation_axis_w, rotation_axis_r, rotation_axis_l;
            Eigen::Vector3d arm_in_obj_r, arm_in_w_r, arm_in_wrist_r, wrist_in_obj_r, proj_in_obj_r;
            Eigen::Vector3d arm_in_obj_l, arm_in_w_l, arm_in_wrist_l, wrist_in_obj_l, proj_in_obj_l;

            wrist_in_obj_r = Obj_orientation_temp_b.e().transpose()*rel_objpalm_r.e();
            proj_in_obj_r.setZero();
            proj_in_obj_r[2] = wrist_in_obj_r[2];
            arm_in_obj_r.setZero();
            arm_in_obj_r[0] = -wrist_in_obj_r[0];
            arm_in_obj_r[1] = -wrist_in_obj_r[1];
            arm_in_obj_r[2] = 0;
            arm_in_w_r = Obj_orientation_temp_b.e() * arm_in_obj_r;
            arm_in_wrist_r = wrist_ori_r.e().transpose() * arm_in_w_r;
            ...
            Eigen::VectorXd arm_length_r, arm_length_l;
            arm_length_r.setZero(1);
            arm_length_l.setZero(1);
            arm_length_r[0] = arm_in_wrist_r.norm();
            arm_length_l[0] = arm_in_wrist_l.norm();

            rotation_axis_obj.setZero();
            rotation_axis_obj[2] = 1;
            rotation_axis_w = Obj_orientation_temp_b.e() * rotation_axis_obj;
            rotation_axis_r = wrist_ori_r.e().transpose() * rotation_axis_w;
            rotation_axis_l = wrist_ori_l.e().transpose() * rotation_axis_w;


            Eigen::VectorXd obj_pose6d_r_, obj_pose6d_l_;
            obj_pose6d_r_.setZero(6);
            obj_pose6d_l_.setZero(6);

            raisim::Mat<3,3> obj_bottom_init_mat, obj_bottom_init_mat_trans, obj_bottom_diff_mat, obj_bottom_diff_wrist_mat_r, obj_bottom_diff_wrist_mat_l;

            raisim::quatToRotMat(obj_pos_init_b_.segment(3,4), obj_bottom_init_mat);
            raisim::transpose(obj_bottom_init_mat, obj_bottom_init_mat_trans);
            raisim::matmul(obj_bottom_init_mat_trans, Obj_orientation_temp_t, obj_bottom_diff_mat);
            raisim::matmul(palm_world_pose_mat_trans_r, obj_bottom_diff_mat, obj_bottom_diff_wrist_mat_r);
            raisim::matmul(palm_world_pose_mat_trans_l, obj_bottom_diff_mat, obj_bottom_diff_wrist_mat_l);

            obj_pose6d_r_.segment(0,3) = obj_bottom_diff_wrist_mat_r.e().col(0);
            obj_pose6d_r_.segment(3,3) = obj_bottom_diff_wrist_mat_r.e().col(1);
            obj_pose6d_l_.segment(0,3) = obj_bottom_diff_wrist_mat_l.e().col(0);
            obj_pose6d_l_.segment(3,3) = obj_bottom_diff_wrist_mat_l.e().col(1);

            Eigen::Vector3d z_world, z_world_wrist_r, z_world_wrist_l;
            raisim::Mat<3,3> hand_ori_mat_r, hand_ori_mat_l;
            z_world.setZero();
            z_world_wrist_r.setZero();
            z_world[2] = 1;
            mano_l_->getFrameOrientation(body_parts_l_[0], hand_ori_mat_l); // Body_orientation:
            mano_r_->getFrameOrientation(body_parts_r_[0], hand_ori_mat_r); // Body_orientation:
            z_world_wrist_l = hand_ori_mat_l.e().transpose()*z_world;
            z_world_wrist_r = hand_ori_mat_r.e().transpose()*z_world;

            Eigen::VectorXd obj_weight_vec;
            obj_weight_vec.setZero(2);
            obj_weight_t_ = arctic->getMass(top_id);
            obj_weight_b_ = arctic->getMass(bottom_id);
            obj_weight_ = obj_weight_b_ + obj_weight_t_;
            obj_weight_vec[0] = obj_weight_b_;
            obj_weight_vec[1] = obj_weight_t_;

            obDouble_r_ << gc_r_.tail(gcDim_ - 6),      // (mirror) 45, generalized coordinate
                    ...
                    rel_body_pos_r_,    //  (x mirror) 63, joint position relative to target position in wrist coord, # TODO: relative to frame
                    rel_pose_r_,  // (need to mirror the first 3 dimension) 48, angle between current pose and final pose, wrist pose in object coord # TODO: relative to frame
                    ...
                    arm_in_wrist_r,
                    arm_length_r,  //norm of arm
                    obj_weight_vec, //2, obj mass of bottom and top
                    rotation_axis_r, // rotation axis in wrist frame
            ...
            raisim::Vec<3> trans_r, trans_l;
            raisim::Mat<3,3> rot_r, rot_l;
            raisim::Vec<4> quat_r, quat_l;
            ...
            mano_l_->getFramePosition(body_parts_l_[0], trans_l); // Position: body position of a body in mano
            mano_l_->getFrameOrientation(body_parts_l_[0], rot_l); // Body_orientation:
            raisim::rotMatToQuat(rot_r, quat_r);
            raisim::rotMatToQuat(rot_l, quat_l);

            Eigen::VectorXd obj_state;
            obj_state.setZero(8);
            obj_state = arctic->getGeneralizedCoordinate().e();

             global_state_ << obj_goal_pos_b_dis_.norm(),
                             rel_obj_pose_world_b_.norm(),
                             rel_obj_init_b.norm(),
                             obj_angle_,
                             rel_obj_angle_,
                             rel_obj_init_b[2],
        ...
        void control_switch(int left, int right) {
            ...
            if (left == 1){
                actionMean_l_bias.head(6) += actionMean_l_bk.head(6);
                root_guiding_counter_l_ = 0;
                Eigen::VectorXd jointPgain(gvDim_), jointDgain(gvDim_);
                root_guided_l = false;
                root_keep_l = false;
                imitate_l = false;
                imitate_l_world = false;
                motion_synthesis_l = false;
                ms_l = true;
                ...
                jointDgain.head(3).setConstant(3.0);
                jointPgain.tail(nJoints_).setConstant(50.0);
                jointDgain.tail(nJoints_).setConstant(0.2);
                mano_l_->setPdGains(jointPgain, jointDgain);
            }
            ...
            // arti
                ...
                imitate_l_world = false;
                motion_synthesis_l = false;
                jointPgain.head(3).setConstant(50.0);
                jointDgain.head(3).setConstant(0.1);
                jointPgain.tail(nJoints_).setConstant(50.0);
                jointDgain.tail(nJoints_).setConstant(0.2);
                mano_l_->setPdGains(jointPgain, jointDgain);
            ...
            if (right == 1){
                actionMean_r_bias.head(6) += actionMean_r_bk.head(6);
                root_guiding_counter_r_ = 0;
                motion_synthesis = false;
                root_guided_r = false;
                root_keep_r = false;
                imitate_r = false;
                imitate_r_world = false;
                ms_r = true;
                Eigen::VectorXd jointPgain(gvDim_), jointDgain(gvDim_);
                jointPgain.head(3).setConstant(200);
                jointDgain.head(3).setConstant(3.0);
        ...
        void set_imitation_goals(const Eigen::Ref<EigenVec>& pose_l,
                                 const Eigen::Ref<EigenVec>& pose_r,
                                 const Eigen::Ref<EigenVec>& obj_pose)
        {
            raisim::Vec<3> pos_r_w, pos_l_w, pos_obj_w;
            raisim::Vec<3> pos_r_o, pos_l_o, pos_r_diff, pos_l_diff;
            raisim::Vec<3> euler_r_w, euler_l_w, euler_r_o, euler_l_o;
            raisim::Vec<4> quat_l_w, quat_r_w, quat_l_o, quat_r_o, ori_obj;
            raisim::Mat<3,3> mat_r_o, mat_l_o, mat_obj, mat_obj_trans;
            pos_l_w = pose_l.head(3);
            euler_l_w = pose_l.segment(3,3);
            pos_r_w = pose_r.head(3);
            euler_r_w = pose_r.segment(3,3);
            pos_obj_w = obj_pose.head(3);
            ori_obj = obj_pose.segment(3,4);

            pos_r_diff = pos_r_w - pos_obj_w;
            pos_l_diff = pos_l_w - pos_obj_w;

            raisim::quatToRotMat(ori_obj, mat_obj);
            raisim::transpose(mat_obj, mat_obj_trans);

            raisim::matvecmul(mat_obj_trans, pos_r_diff, pos_r_o);
            raisim::matvecmul(mat_obj_trans, pos_l_diff, pos_l_o);

            raisim::eulerToQuat(euler_l_w, quat_l_w);
            raisim::eulerToQuat(euler_r_w, quat_r_w);
            raisim::quatInvQuatMul(ori_obj, quat_l_w, quat_l_o);
            raisim::quatInvQuatMul(ori_obj, quat_r_w, quat_r_o);
            raisim::quatToRotMat(quat_l_o, mat_l_o);
            raisim::quatToRotMat(quat_r_o, mat_r_o);
            raisim::RotmatToEuler(mat_l_o, euler_l_o);
            raisim::RotmatToEuler(mat_r_o, euler_r_o);

            imitation_pose_l_ = pose_l.cast<double>();
            imitation_pose_l_.head(3) = pos_l_o.e();
            imitation_pose_l_.segment(3,3) = euler_l_o.e();
            imitation_pose_r_ = pose_r.cast<double>();
            imitation_pose_r_.head(3) = pos_r_o.e();
            imitation_pose_r_.segment(3,3) = euler_r_o.e();

            gc_world_l_ = pose_l.cast<double>();
            gc_world_r_ = pose_r.cast<double>();
            gc_world_l_.head(6) = gc_l_.head(6);
            gc_world_r_.head(6) = gc_r_.head(6);

            raisim::Vec<3> act_or_pose_r, act_or_pose_l;
            raisim::matvecmul(init_or_r_, pose_r.head(3), act_or_pose_r); // rotate the world coordinate into hand's origin frame (from the start of the episode)
            gc_world_r_.head(3) += act_or_pose_r.e();
            raisim::matvecmul(init_or_l_, pose_l.head(3), act_or_pose_l); // rotate the world coordinate into hand's origin frame (from the start of the episode)
            gc_world_l_.head(3) += act_or_pose_l.e();
    ...
    private:
        ...
        Eigen::VectorXd gc_r_imitate, gc_l_imitate;
        Eigen::VectorXd freezedTarget_l_, freezedTarget_obj_, freezedTarget_r_;
        ...
        Eigen::VectorXd obj_goal_pos_b_dis_, obj_goal_pos_t_dis_;
        ...
        raisim::Box *box_obj;
        raisim::Box *box;
        raisim::Box *stage, *stage2;
        raisim::ArticulatedSystem *arctic, *arctic_bk, *mano_l_, *mano_r_;
        raisim::ArticulatedSystemVisual *arcticVisual, *arcticVisual2;
        int rf_dim = 6;
        int num_obj = 4;
        int num_contacts = 16;
        int num_joint = 17;
        ...
        int obj_idx_;
        int imitate_count_l = 0;
        int imitate_count_r = 0;
        ...
        bool ms_l = false;
        bool ms_r = false;
        bool imitate_l = false;
        bool imitate_r = false;
        bool imitate_l_world = false;
        bool imitate_r_world = false;
        Eigen::VectorXd gc_world_r_, gc_world_l_;
        float rewards_sum_[2];
        bool stage_flag = true;
        int iter_num;
        raisim::Vec<3> pose_goal_r, pos_goal_r, up_vec_r, up_gen_vec_r, obj_pose_r_, Position_r, Obj_Position_b, Rel_fpos, Obj_linvel_b, Obj_qvel_b, Fpos_world_r, palm_world_pos_r_, init_root_r_, init_obj_, init_obj_2;
        ...
        raisim::Vec<3> obj_euler_b_, obj_euler_t_;
        raisim::Vec<3> vel_l, vel_r, angular_vel_l, angular_vel_r, obj_trans_init_b;
        raisim::Mat<3,3> Obj_orientation_b, Obj_orientation_t, Obj_orientation_temp_b, Obj_orientation_temp_t, Body_orientation_r, Body_orientation_l, init_or_r_, init_or_l_, root_pose_world_r_, root_pose_world_l_, init_rot_r_, init_rot_l_, init_obj_rot_;
        ...
        raisim::Vec<3> rel_obj_pose_world_b_, rel_obj_pose_world_t_;
        raisim::Vec<3>  rel_obj_pos_wrist_l_, rel_obj_pos_wrist_r_;
        std::vector<int> contact_idxs_;
        Eigen::Vector3d stage_dim, stage2_dim;
        Eigen::Vector3d stage_pos, stage2_pos;
